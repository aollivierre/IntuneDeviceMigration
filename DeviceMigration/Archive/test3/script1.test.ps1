...
while (-not (Test-LockExpired -LockFile $lockFile -Timeout $lockTimeout)) {
    Write-Host "Script 1: Lock is active. Waiting for it to be released..."
    Start-Sleep -Seconds 5
}

# Create the lock file
New-Item -Path $lockFile -ItemType File | Out-Null
Write-Host "Script 1: Acquired the lock."

try {
    # Simulate module installation and import
    Write-Host "Script 1: Installing and importing modules..."
    ...
    Write-Host "Script 1: Modules installed and imported successfully."
}
finally {
    # Release the lock
    Remove-Item $lockFile -Force
    Write-Host "Script 1: Lock released."
}
