...
Describe "Get-LockingProcess Function Tests" {

    Context "When Handle executable is not found" {

        It "should throw an error and log the appropriate message" {
            Mock -CommandName Test-Path -MockWith { return $false }

            { Get-LockingProcess -FilePath "C:\Temp\TestFile.txt" -HandlePath "C:\Invalid\Handle.exe" } |
                Should -Throw -ErrorMessage "Handle executable not found at path"
        }
    }
}
