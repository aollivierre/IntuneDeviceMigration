...
if ($dsregStatus.IsAzureADJoined -or $dsregStatus.IsHybridJoined) {
    if ($dsregStatus.IsMDMEnrolled) {
        exit 0 # Device is either Azure AD or Hybrid joined and Intune enrolled
    } else {
        exit 1 # Device is Azure AD or Hybrid joined but not Intune enrolled
    }
} else {
    exit 2 # Device is neither Azure AD nor Hybrid joined (i.e., Workgroup or On-prem only)
}
